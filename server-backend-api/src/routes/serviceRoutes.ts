...
router.get(apiUrlBuilderV1.createUrlAll(resource), async (req, res) => {
    try {
        const servicesDb : ServiceType[] | null = await serviceController.getAll();
...
router.get(apiUrlBuilderV1.createUrlWithId(resource), async (req, res) => {
    try {
        const {id} = req.params;
        if (!isNumberString(id)) return sendError(req, res, 300, 'Bad Request')
...
router.post(apiUrlBuilderV1.createUrlAdd(resource), async (req, res) => {
    try {
        ...
        if (err instanceof z.ZodError){
            return sendError(req, res, 400, `Bad request: ${generateZodErrorString(err)}`)
        }
        sendError(req, res);
    }
});
